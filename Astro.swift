...
class Astro {
    ...
    // coming soon
    // map (alt,az) -> (ra,dec) for a give time and location
    class func raDec(altitude:Double, azimuth:Double, date:Date, latitude:Double, longitude:Double) -> (ra:Double,dec:Double) {

            print("ERROR raDec() is not supported")
            return(ra:0,dec:0)
    }
